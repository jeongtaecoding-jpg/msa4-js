...
// inline 방식(매우 비추천!!) HTML 문서의 인라인인라인을 참조
// 이벤트 대상인 요소의 속성으로 이벤트를 지정하는 방법
// HTML문서에 JS가 혼재되므로 관리상의 문제가 발생


// property Listener 방식(잘 안씀)
...
//   console.log('스크롤 실행');
// });   이렇게 하면 주소 값이 달라서 스크롤 실행 제거를 할 수 없다.

// window.removeEventListener('scroll', printScroll);
// printScroll 의 주소는 두 개다 똑같기 때문에 제거가 가능하다.
...
window.addEventListener('scroll', e => printScroll);
function printScroll(e) {
  console.log(`스크롤 실행: ${e}`);
}
